...
CREATE TABLE "mcp_server_variable" (
	"id" text PRIMARY KEY NOT NULL,
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "mcp_server_variable_server_name_unique" UNIQUE("server_id","name")
);
--> statement-breakpoint
...
ALTER TABLE "mcp_server_variable" ADD CONSTRAINT "mcp_server_variable_server_id_mcp_server_id_fk" FOREIGN KEY ("server_id") REFERENCES "public"."mcp_server"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "mcp_server_variable_server_id_idx" ON "mcp_server_variable" USING btree ("server_id");--> statement-breakpoint
...
CREATE INDEX "mcp_call_log_created_at_idx" ON "mcp_call_log" USING btree ("created_at");