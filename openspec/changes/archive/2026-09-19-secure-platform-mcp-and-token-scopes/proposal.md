...
- **BREAKING**: delete existing development Platform PATs and their legacy JSON grants, remove the singleton-token APIs, and require new normalized PATs. Server-scoped gateway tokens are a separate credential type and are not changed by this capability.
...
- Drizzle token storage, normalized scope/server grants, security-event retention, a clean generated migration that removes legacy Platform grant storage, and updated seeds.
