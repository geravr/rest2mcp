...
- **MCP Draft vs Published Ownership**: Saving an MCP server edit (tools, bindings, base URL, structural auth, non-secret config) changes only the owner's mutable draft and never agent behavior. Runtime discovery and execution read exactly one immutable active `publishedRevisionId` revision; no code path may read mutable `mcp_server` / `mcp_tool` / `mcp_server_variable` rows as runtime config, backfill a revision automatically, or keep a dual-read/shadow/compatibility fallback. Operational actions (pause/resume, token revocation, secret-material rotation, presentation-only icon changes) take effect immediately and do not dirty the draft. While `PRE_PRODUCTION`, reset or reseed disposable development databases (`bun db:seed`) instead of adding compatibility paths for retained drafts; no exception is granted for legacy authoring paths.
