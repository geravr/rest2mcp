export const enServers = {
  servers: {
    nav: "Servers",
    title: "Servers",
    description: "Map REST APIs to hosted MCP tools.",
    create: "Create server",
    creating: "Creating…",
    emptyTitle: "No servers yet",
    emptyDescription:
      "Create a server, add a GET tool, and copy a connection snippet.",
    name: "Name",
    slug: "Slug",
    baseUrl: "Base URL",
    descriptionLabel: "Description",
    status: "Status",
    traffic: "Health",
    tools: "Tools",
    settings: "Settings",
    connection: "Connection",
    playground: "Playground",
    logs: "Logs",
    tabsWork: "Build and test",
    tabsSetup: "Connect and configure",
    pause: "Pause",
    resume: "Resume",
    pausing: "Updating…",
    backToList: "All servers",
    notFound: "This server was not found.",
    createTitle: "New server",
    createDescription:
      "Give it a name and the HTTPS origin of the API you want to wrap.",
    namePlaceholder: "CRM",
    baseUrlPlaceholder: "https://api.example.com",
    optionalDescription: "Optional description",
    cancel: "Cancel",
    trafficDraft: "Draft",
    trafficGreen: "Healthy",
    trafficYellow: "Unstable",
    trafficRed: "Failing",
    trafficPaused: "Paused",
    toolCount: "{count} tools",
    toolCountOne: "1 tool",
    addTool: "Add tool",
    importCurl: "Import curl",
    toolName: "Tool name",
    toolNamePlaceholder: "get_contact",
    toolTitle: "Title",
    toolTitlePlaceholder: "Get contact",
    method: "Method",
    path: "Path",
    toolPathSummary: "Path",
    pathPlaceholder: "/contacts/",
    allowMutation: "Allow mutation",
    enabled: "Enabled",
    selectAllTools: "Select all tools on this page",
    toolCap: "This server already has {count} tools; the limit is {limit}.",
    curlLabel: "curl command",
    curlPlaceholder: "curl https://api.example.com/v1/items",
    saveTool: "Save tool",
    savingTool: "Saving…",
    noTools: "No tools yet. Add a GET tool or paste a curl command.",
    groups: {
      filterLabel: "Filter by group",
      all: "All",
      ungrouped: "Ungrouped",
      toolCount: "{count} tools",
      toolCountOne: "1 tool",
      createTitle: "New group",
      createDescription:
        "Groups organize tools in Studio. They do not change tool configuration or published agent behavior.",
      renameTitle: "Rename group",
      renameDescription:
        "Renaming only changes the Studio label. Tool configuration and published agent behavior are unchanged.",
      deleteTitle: "Delete group",
      deleteDescription:
        "Deleting {name} removes only the group. Its {count} tools move to Ungrouped — the tools themselves are not deleted, and their configuration and published agent behavior are unchanged.",
      deleteDescriptionOne:
        "Deleting {name} removes only the group. Its single tool moves to Ungrouped — the tool itself is not deleted, and its configuration and published agent behavior are unchanged.",
      nameLabel: "Group name",
      namePlaceholder: "Invoices",
      nameHint: "Up to {limit} characters.",
      limitReached:
        "This server already has {limit} groups, the current limit.",
      create: "Create group",
      creating: "Creating…",
      rename: "Rename group",
      renaming: "Renaming…",
      delete: "Delete group",
      deleting: "Deleting…",
      moveTitle: "Move tools",
      moveDescription:
        "Move the selected tool to a group. Moving changes only Studio organization — it does not change tool configuration or published agent behavior.",
      moveTarget: "Group",
      moveSubmit: "Move tools",
      moving: "Moving…",
      created: "Group created.",
      renamed: "Group renamed.",
      deleted: "Group deleted. Its tools are now ungrouped.",
      toolsMoved: "Tool organization updated.",
      railLabel: "Groups",
      groupActions: "Actions for {group}",
      searchLabel: "Search tools",
      searchPlaceholder: "Search tools…",
      moveTo: "Move to…",
      inGroupHeader: "Tools in {group}",
      emptyGroup:
        "This group has no tools yet. Select tools and move them here.",
      emptyUngrouped:
        "Every tool belongs to a group. Select tools and remove them from their group.",
      searchEmpty: 'No tools match "{query}".',
    },
    identityTitle: "Server",
    identityDescription: "Name, origin, and optional notes for this server.",
    iconTitle: "Icon",
    iconDescription:
      "Upload a custom icon or use the automatic rings placeholder.",
    iconUpload: "Upload icon",
    iconUploading: "Uploading…",
    iconRemove: "Remove custom icon",
    iconRemoving: "Removing…",
    iconPhotoDescription: "Upload a JPG, PNG, or WebP image up to 5 MB.",
    invalidIconType: "Upload a JPG, PNG, or WebP image.",
    iconTooLarge: "Icon images must be 5 MB or smaller.",
    iconUploadFailed: "The icon upload could not be finalized. Try again.",
    variables: "Server values",
    variablesDescription:
      "Configuration and secrets tools reference by name. Secret values are encrypted and never shown again.",
    noVariables:
      "No server values yet. Add a secret for API keys so they stay out of tool definitions.",
    variableName: "Variable name",
    variableNamePlaceholder: "api_token",
    variableNameHint:
      "Starts with a lowercase letter. Letters, digits, and underscores only.",
    variableInvalidName: "Use a lowercase name like api_token.",
    variableSecretHelp: "Encrypted at rest. You will not see this value again.",
    variableValue: "Value",
    variableSecret: "Secret",
    variableSecretBadge: "Secret",
    variableConfigBadge: "Config",
    variableAuthOwnedBadge: "Managed by Authentication",
    variableManualOwnerBadge: "Manual",
    variableHasValue: "Value stored",
    variableNoValue: "No value stored",
    addVariable: "Add variable",
    addingVariable: "Saving…",
    deleteVariable: "Delete",
    editVariable: "Edit",
    editVariableTitle: "Edit variable",
    editVariableSecretHidden:
      "Stored value is hidden. Enter a new value to rotate.",
    editVariableSecretPlaceholder: "sk_live_…",
    editVariablePlaintextHint:
      "Enter the current value to store it in plain text.",
    deleteVariableTitle: "Delete variable",
    deleteVariableDescription: "This permanently deletes {name}.",
    deleteVariableReferencedCommonHeader:
      "May be used in common header {key}. Calls may fail until you edit that row.",
    deleteVariableReferencedCommonQuery:
      "May be used in common query param {key}. Calls may fail until you edit that row.",
    deleteVariableReferencedTools:
      "May be used in tools on this page: {names}.",
    originFixed: "Fixed",
    originVariable: "Variable",
    originAgent: "Agent",
    originPrefix: "Prefix",
    originPrefixPlaceholder: "Bearer ",
    originPrefixHint: "Optional text before the variable, for example Bearer ",
    originSelect: "Value origin",
    variablePickerPlaceholder: "Select variable",
    variablePickerEmpty: "No variables yet. Create them in Settings.",
    pathInsertVariable: "Insert variable",
    pathInsertAgent: "Insert agent param",
    pathTokenRemove: "Remove token",
    bodyAdvanced: "Advanced",
    bodyFields: "Fields",
    bodyAdvancedHint:
      "Insert a variable, or type a name for the agent to fill.",
    bodyNoneMessage: "This request has no body.",
    emptyQueryRows: "No query params yet.",
    emptyHeaderRows: "No headers yet.",
    emptyFormRows: "No body fields yet.",
    agentParamTitle: "Agent param",
    agentParamNameSaved: "Agent argument {name}",
    discardToolTitle: "Discard this tool?",
    discardToolDescription:
      "Your changes will be lost if you close without saving.",
    discardToolConfirm: "Discard",
    insertVariable: "Insert variable",
    requestPartQuery: "Query",
    requestPartHeaders: "Headers",
    requestPartBody: "Body",
    toolDialogDescription:
      "Name the tool and build the request the agent will call.",
    defaultsTitle: "Common values",
    defaultsDescription:
      "Sent on every tool call. Use a server value for secrets.",
    commonHeaders: "Common headers",
    commonQuery: "Common query params",
    saveDefaults: "Save common values",
    savingDefaults: "Saving…",
    defaultsUnsaved: "Unsaved changes",
    snippetTitle: "Connection snippet",
    snippetDescription:
      "Paste this URL into an MCP client and send the agent token as Bearer. Upstream 4xx/5xx return as MCP tool errors with a structured envelope; curl import never stores credentials — configure Auth separately.",
    createToken: "Create agent token",
    creatingToken: "Creating…",
    tokenShownOnce: "Copy this token now. It will not be shown again.",
    copy: "Copy",
    copied: "Copied",
    noTokens: "No agent tokens yet.",
    revoke: "Revoke",
    revoking: "Revoking…",
    prefix: "Prefix",
    invoke: "Invoke",
    invoking: "Invoking…",
    result: "Result",
    selectTool: "Select a tool",
    invalidJson: "Arguments must be valid JSON.",
    invalidNumber: "Must be a number.",
    noLogs: "No calls logged yet.",
    source: "Source",
    duration: "Duration",
    httpStatus: "HTTP",
    created: "When",
    request: "Request",
    response: "Response",
    viewLog: "View",
    logDetail: "Call detail",
    logDetailDescription: "Redacted request and response stored for this call.",
    appCode: "Code",
    neverCalled: "No calls yet",
    copyMcpUrl: "Copy MCP URL",
    deleteServer: "Delete server",
    testingConnection: "Testing…",
    connectionReachable: "Reachable — the API answered (HTTP {status}).",
    connectionAuthFailing:
      "Reachable, but auth is failing (HTTP {status}). Check your credentials.",
    connectionUnreachable: "Unreachable — {detail}",
    createdNextStep: "Server created. Next: add your first tool.",
    addFirstTool: "Add the first tool",
    testConnection: "Test connection",
    authType: "Authentication",
    authTypeNone: "None",
    authTypeBearer: "Bearer token",
    authTypeHeader: "API key header",
    authTypeQuery: "API key query",
    authTypeBasic: "Basic",
    authTypeCustom: "Custom",
    authToken: "Token",
    authTokenPlaceholder: "Paste token",
    authHeaderName: "Header name",
    authHeaderNamePlaceholder: "X-API-Key",
    authHeaderValue: "Header value",
    authParamName: "Query parameter",
    authParamNamePlaceholder: "api_key",
    authParamValue: "Parameter value",
    authValuePlaceholder: "Paste secret",
    authQueryAckLabel:
      "I understand this credential will appear in URLs, server logs, and browser history.",
    authUsername: "Username",
    authPassword: "Password",
    authCustomHelp:
      "This server uses a custom auth setup. Edit default headers or query below, or pick a simple type to replace the mapped credential.",
    authProtectedKeysLabel:
      "Keys managed by authentication (protected from tool and default overrides):",
    authTitle: "Authentication",
    authDescription:
      "Credentials are stored as encrypted secrets. You will not see the value again after saving.",
    authSave: "Save authentication",
    authSaving: "Saving…",
    authRequiredFields: "Fill in the required authentication fields.",
    authSaved: "Authentication structure saved to the draft.",
    existingAuthKept:
      "Tool created. Existing server authentication was kept — the curl credential was not stored.",
    invokeDisabledTool: "This tool is disabled. Enable it in Tools to invoke.",
    invokeMutationBlocked:
      "Mutations are not allowed for this tool. Enable allow mutation in Tools.",
    invokeServerPaused: "This server is paused. Resume it to invoke tools.",
    resultStatus: "HTTP {status}",
    resultError: "The upstream request did not complete successfully.",
    saveChanges: "Save changes",
    savingChanges: "Saving…",
    dangerZoneTitle: "Danger zone",
    dangerZoneDescription: "Irreversible actions for this MCP server.",
    deleteTitle: "Delete server",
    deleteDescription:
      "This permanently deletes {name} with its tools, variables, tokens, and call history.",
    deleteConfirmLabel: "Type {name} to confirm",
    deleteConfirm: "Delete permanently",
    deleting: "Deleting…",
    editTool: "Edit",
    editToolTitle: "Edit tool",
    duplicateTool: "Duplicate",
    deleteTool: "Delete",
    deleteToolTitle: "Delete tool",
    deleteToolDescription: "This deletes {name}. Past call logs are kept.",
    toolDescription: "Description",
    toolDescriptionPlaceholder: "What does this tool do? Shown to the agent.",
    queryParams: "Query params",
    headers: "Headers",
    bodyType: "Body type",
    bodyTypes: {
      none: "None",
      json: "JSON",
      form: "Form",
      raw: "Raw",
    },
    bodyJsonPlaceholder: '{ "name": "{{name}}" }',
    bodyRawPlaceholder: "Raw body with optional {{placeholders}}",
    allowMutationHelp:
      "Off: the tool stays read-only even for POST, PUT, PATCH, or DELETE.",
    enabledHelp: "Disabled tools are hidden from agents.",
    paramDescriptionPlaceholder: "What is this value? Shown to the agent.",
    paramDescription: "Param description",
    paramRequired: "Required",
    paramType: "Param type",
    paramTypes: {
      string: "Text",
      number: "Number",
      integer: "Integer",
      boolean: "Boolean",
      json: "JSON",
      array: "Array",
    },
    paramFormat: "Input format",
    paramFormats: {
      none: "None",
      date: "Date",
      "date-time": "Date & time",
      email: "Email",
      uri: "URL",
      uuid: "UUID",
    },
    paramSensitive: "Sensitive",
    paramConstraints: "Constraints",
    paramMinLength: "Min length",
    paramMaxLength: "Max length",
    paramPattern: "Pattern (regex)",
    paramPatternPlaceholder: "^[A-Z]{2}\\d+$",
    paramMinimum: "Minimum",
    paramMaximum: "Maximum",
    paramMinItems: "Min items",
    paramMaxItems: "Max items",
    paramItemType: "Item type",
    paramUniqueItems: "Unique items",
    paramExamples: "Examples",
    paramExamplesPlaceholder: "value1, value2",
    kvKeyPlaceholder: "Key",
    kvValuePlaceholder: "Value",
    kvAddRow: "Add row",
    kvRemoveRow: "Remove row",
    curlParse: "Parse",
    curlParsing: "Parsing…",
    curlPreviewDescription:
      "Mark each highlighted value as an agent input, or leave it literal.",
    locations: {
      path: "Path",
      query: "Query",
      header: "Header",
      json: "Body",
      form: "Body",
    },
    markAsParam: "Agent input",
    markAsLiteral: "Literal",
    markingAs: "Mark as",
    markingName: "Agent input name",
    markingNamePlaceholder: "name",
    invalidMarkingName:
      "Invalid name: use a lowercase letter followed by lowercase letters, digits, or underscores.",
    credentialExcluded:
      "Detected credential ({kind}, {header}) was excluded. Configure authentication separately in the Auth card.",
    curlCreate: "Create tool",
    curlBack: "Back",
    curlImportSummary:
      "Draft tool created and disabled. {issues} validation issue(s) found — review them before enabling it.",
    curlImportCredentialsCta:
      "This curl command included credentials. They were never stored — configure authentication separately below.",
    curlGoToAuth: "Go to Authentication",
    viewCallLog: "View call log",
    previewTitle: "Effective request preview",
    previewDescription:
      "Compiles the request the same way the gateway will, using current server values and authentication. Secret values are never shown.",
    previewRun: "Preview",
    previewRunning: "Compiling…",
    previewValid: "This request compiles without issues.",
    previewSecretValue: "•••• (secret value)",
    previewAgentInput: "agent input",
    previewQueryRepeated: "repeated keys",
    previewQueryDelimited: "comma-delimited",
    previewContractHeading: "Agent contract",
    previewContractName: "Name",
    previewContractTitleLabel: "Title",
    previewContractDescription: "Description",
    previewContractVersion: "Contract version",
    previewContractFingerprint: "Fingerprint",
    previewContractInputSchema: "Input schema",
    previewContractNoProperties:
      "No agent inputs. This tool is called with an empty object.",
    previewContractRequired: "required",
    previewContractOptional: "optional",
    previewContractOutputSchema: "Output schema",
    previewContractOutputPresent: "Structured output is advertised.",
    previewContractOutputAbsent: "No structured output schema.",
    previewContractAnnotations: "Annotations",
    previewSavedIssues:
      "This tool has {count} unresolved compile issue(s) from the last save. Run Preview to see details.",
    advancedJsonInvalid:
      "The advanced JSON body is not valid JSON. Fix it before previewing or saving.",
    compileInvalidShort: "Fix compile issues to enable",
    annotationReadOnly: "Read-only",
    annotationDestructive: "Destructive",
    annotationIdempotent: "Idempotent",
    annotationOpenWorld: "Open world",
    annotationsTitle: "Agent behavior",
    annotationsHelp:
      "Describe how this tool behaves so agents can decide when to call it. Read-only and open world follow the HTTP method and the REST upstream.",
    annotationDestructiveHelp: "This call can delete or overwrite data.",
    annotationIdempotentHelp:
      "Repeating this call has the same effect as calling it once.",
    annotationReadOnlyHelp:
      "Read requests are always read-only, non-destructive, and idempotent.",
    contractRetryAllowsRetry:
      "The agent may retry this call after a transient failure.",
    contractRetryNotAutomatic:
      "The agent will not be told to retry this call automatically.",
    mutationConfirmTitle: "Revoke mutation permission?",
    mutationConfirmDescription:
      "This tool sends a mutating request. Turning off Allow mutation will also disable it.",
    mutationConfirmConfirm: "Revoke and disable",
    enabledBlockedHelp:
      "This tool cannot be enabled until the compile issues above are resolved.",
    unpublishedChanges: "Unpublished changes",
    publicationBlocked: "Publication blocked",
    publishedRevision: "Published revision {number}",
    neverPublished: "Not published yet — agents cannot call this server.",
    publishChanges: "Publish changes",
    publishingChanges: "Publishing…",
    publishTitle: "Publish review",
    publishDescription:
      "Review what agents will see, then create an immutable published revision.",
    publishUnavailable: "The publication preview is unavailable.",
    publishStale:
      "The draft or published revision changed after this preview. Review the refreshed changes before publishing.",
    publishRefresh: "Refresh preview",
    unpublishedRevision: "No published revision",
    publishReady: "Ready to publish",
    publishBlocked: "Blocked by errors",
    publishNoChanges: "No publishable changes",
    publishBlockingErrors: "Resolve these blocking errors:",
    publishWarningsTitle: "Acknowledge these warnings:",
    publishWarnings: {
      contract_changed:
        "The enabled agent contract changes with this publication.",
      tool_added: "One or more tools are added to the agent contract.",
      tool_removed: "One or more published tools are removed.",
      tool_disabled: "One or more published tools are disabled.",
      destructive_change:
        "This publication changes mutating or destructive behavior.",
      auth_changed: "Structural authentication configuration changes.",
      config_changed: "Non-secret configuration values change.",
    },
    publishIssueFallback: "Unresolved issue ({code}).",
    publishDiffTitle: "What changes",
    publishDiffServer: "Server routing",
    publishDiffAuth: "Authentication structure",
    publishDiffCommon: "Default headers or query",
    publishDiffToolsAdded: "Tools added",
    publishDiffToolsRemoved: "Tools removed",
    publishDiffToolsChanged: "Tools changed",
    publishDiffToolsEnabled: "Tools enabled",
    publishDiffToolsDisabled: "Tools disabled",
    publishDiffConfig: "Server values changed",
    publishDiffContract: "Agent contract changed",
    publishDiffDestructive: "Destructive behavior changed",
    publishFingerprint: "Candidate fingerprint {fingerprint}",
    publishNoteLabel: "Publication note (optional)",
    publishNotePlaceholder: "Why are you publishing these changes?",
    publishAckRemaining: "Acknowledge every warning to enable publishing.",
    publishSuccessTitle: "Published",
    publishSuccessDescription:
      "Revision {number} is now live for agents. The draft is clean.",
    publishSuccessDone: "Done",
    revisions: "Revisions",
    revisionNumber: "Revision",
    revisionNote: "Note",
    noRevisions: "No published revisions yet.",
    revisionTitle: "Revision {number}",
    revisionDetailTitle: "Revision detail",
    revisionDetailDescription:
      "Immutable snapshot recorded at publication. Secret values are never shown.",
    revisionNotFound: "This revision was not found.",
    revisionActive: "Active",
    revisionActorStudio: "Studio",
    revisionActorPlatform: "Platform",
    revisionCandidateFingerprint: "Candidate fingerprint {fingerprint}",
    revisionContractFingerprint: "Contract fingerprint {fingerprint}",
    revisionToolsTitle: "Tools",
    revisionNoTools: "This revision recorded no tools.",
    revisionIssueCount: "{count} compile issue(s)",
    revisionDisabled: "Disabled",
    revisionConfigTitle: "Server values",
    revisionNoConfig: "This revision recorded no server values.",
    revisionSecretUnavailable: "Missing secret",
    revisionMissingSecrets:
      "This revision references secret values that no longer exist. Restoring creates an editable draft with a blocking issue.",
    restoreToDraft: "Restore to draft",
    restoreConfirmDescription:
      "Restoring copies this revision into the draft. Agent behavior does not change until you preview and publish a new revision.",
    restoreConfirm: "Restore to draft",
    restoring: "Restoring…",
    invokeNoPublishedRevision:
      "This server has no published revision. Publish changes or use draft preview.",
    playgroundNoPublishedTools:
      "The active published revision exposes no callable tools.",
    playgroundMode: "Execution source",
    playgroundModePublished: "Published",
    playgroundModeDraft: "Draft preview",
    playgroundPublishedSource:
      "Calling the active published revision {number}.",
    playgroundDraftSource:
      "Testing draft revision {revision} as the owner. This is not published behavior.",
    playgroundDraftNotice:
      "Draft preview is owner-only testing. It does not change or impersonate the published revision, and these calls do not affect production health.",
    aiOptimizer: {
      disclosed: {
        tool_names: "Tool names",
        tool_descriptions: "Tool descriptions",
        path_shape: "Path shape",
        query_structure: "Query structure",
        body_structure: "Body structure",
        input_metadata: "Input metadata",
        serialization: "Serialization",
        compile_issue_codes: "Compile issue codes",
        header_presence: "Header names (presence only)",
      },
      mutable: {
        tool_name: "Tool name",
        tool_title: "Tool title",
        tool_description: "Tool description",
        input_name: "Input name",
        input_description: "Input description",
        query_key: "Query key",
        query_serialization: "Query serialization",
        query_omit_when_absent: "Query omission flag",
        query_binding: "Query binding",
        json_field_key: "Body field key",
        json_field_omit_when_absent: "Body field omission flag",
        json_field_binding: "Body field binding",
      },
      immutable: {
        method: "HTTP method",
        path: "Path",
        base_url: "Base URL",
        allowed_hosts: "Allowed hosts",
        headers: "Headers",
        authentication: "Authentication",
        secrets: "Secrets",
        literal_values: "Literal values",
        input_types: "Input types",
        input_constraints: "Input constraints",
        input_requiredness: "Requiredness",
        mutation_permission: "Mutation permission",
        enablement: "Enablement",
        groups: "Groups",
        publication: "Publication",
      },
      advisory: {
        suspected_path: "Suspected path issue",
        suspected_method: "Suspected method issue",
        suspected_auth: "Suspected authentication issue",
        suspected_host: "Suspected host issue",
        suspected_header: "Suspected header issue",
        suspected_secret: "Suspected secret issue",
        suspected_mutation_classification:
          "Suspected mutation classification issue",
        suspected_enablement: "Suspected enablement issue",
        suspected_publication: "Suspected publication issue",
        missing_parameter: "Possibly missing parameter",
        type_mismatch: "Possible type mismatch",
        unsupported_restructuring: "Unsupported restructuring",
      },
      rejectedCodes: {
        unknown_target: "Unknown target",
        duplicate_operation: "Duplicate recommendation",
        out_of_bounds: "Out of bounds",
        forbidden_target: "Forbidden change",
        sensitive_target: "Targets a sensitive input",
        missing_value: "Missing value",
        policy_rejected: "Rejected by policy",
        compile_failed: "Failed to compile",
        security_rejected: "Rejected by security checks",
        name_conflict: "Name conflict",
        schema_invalid: "Invalid structure",
        cross_item_reference: "Cross-item reference",
      },
      itemFailure: {
        model_output_invalid: "The model returned invalid output.",
        no_result: "The model returned no result for this tool.",
        prompt_bounds_exceeded: "The tool exceeds the analysis size limit.",
        provider_transient_failure:
          "The provider failed temporarily. Try again.",
        provider_request_rejected: "The provider rejected the request.",
        provider_credential_invalid: "The provider credential was rejected.",
        cancelled: "Cancelled before analysis finished.",
      },
      ineligibleReason: {
        definition_unparseable: "The canonical definition cannot be parsed.",
        definition_unsupported_body:
          "The body type is not supported for analysis.",
        snapshot_too_large: "The tool exceeds the analysis size limit.",
        compile_blocked: "The tool fails canonical compilation.",
        openapi_blocked: "The operation has a blocking diagnostic.",
      },
      title: "AI optimization",
      description:
        "The model reads each endpoint and proposes English names and descriptions another agent can call. Nothing changes until you accept it.",
      preparing: "Preparing the optimization plan…",
      action: "Optimize with AI",
      actionSelected: "Optimize selected with AI",
      scope: {
        single: "One tool",
        selected: "Selected tools",
        all_eligible: "All eligible tools in this server",
        openapi: "Selected import candidates",
      },
      authorize: {
        model: "Provider and model",
        scope: "Scope",
        eligible: "Items that will be analyzed",
        ineligibleShort: "ineligible",
        tokens: "Estimated tokens (input / output)",
        cost: "Estimated cost",
        costEstimated: "about {cost}",
        costUnknown: "Unavailable (no pricing data)",
        expires: "Plan expires",
        disclosedTitle: "What is sent",
        disclosedBody:
          "Tool names, descriptions, path shape, query/body structure, input metadata, serialization, header names (presence only), and compile issue codes. Never credentials, secrets, secret IDs, literal values, base URLs, source URLs, or raw documents.",
        immutableTitle: "What the AI cannot change",
        immutableBody:
          "Paths, HTTP methods, base URLs, hosts, headers, authentication, secrets, input types and requiredness, mutation permission, enablement, groups, or publication. Structural concerns appear as advisory-only findings.",
        policyVersion: "Policy version",
        mutableTitle: "What the AI may suggest",
        ineligibleBody:
          "{count} selected items cannot be analyzed safely and will be skipped:",
        ineligibleBodyOne:
          "1 selected item cannot be analyzed safely and will be skipped:",
        decline: "Cancel",
        approve: "Start optimization",
        lead: "It will read {count} tools and propose English names and descriptions. Nothing is sent until you start.",
        details: "What is shared and what stays fixed",
      },
      progress: {
        inProgress:
          "Reading endpoints and proposing names. You can leave and come back.",
        narrative: "{ready} of {total} ready. {failed} failed.",
        queued: "Queued",
        running: "Running",
        recommended: "With suggestions",
        failed: "Failed",
        cancelling: "Cancelling…",
        cancel: "Cancel run",
      },
      review: {
        empty: "This run produced no reviewable items.",
        partialFailures:
          "Some tools failed analysis. Successful recommendations are shown below.",
        loadMore: "Load more recommendations",
        close: "Close",
        states: {
          recommended: "Ready to review",
          noChange: "No changes suggested",
          failed: "Analysis failed",
          applied: "Applied",
          cancelled: "Cancelled",
        },
        failureCode: "Analysis failed ({code}).",
        operationsTitle: "Suggested changes",
        fields: {
          set_tool_name: "Name",
          set_tool_title: "Title",
          set_tool_description: "Description",
          set_input_name: "Parameter",
          set_input_description: "Parameter description",
          set_query_entry_key: "Query key",
          set_query_entry_serialization: "Serialization",
          set_query_entry_omit_when_absent: "Omission",
          rebind_query_entry: "Query binding",
          rebind_json_field: "Body binding",
          set_json_field_key: "Body field",
          set_json_field_omit_when_absent: "Field omission",
        },
        selectAllSafe: "Select all",
        clearAll: "Clear selection",
        safe: "Safe",
        guarded: "Guarded",
        advisoriesTitle: "Review manually (not applicable automatically)",
        rejectedTitle: "Rejected suggestions",
        applySelected: "Apply {count} selected to draft",
        savedToDraft: "Saved to the draft. Publish to make it agent-visible.",
      },
      import: {
        reviewHint:
          "Selected recommendations will be applied atomically with the import.",
        applyWithImport: "Include in this import",
      },
    },
  },
} as const;
