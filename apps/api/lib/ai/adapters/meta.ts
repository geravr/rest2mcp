/**
 * Direct adapter for the Meta Model API. Verifies credentials against the
 * models endpoint (an unauthenticated probe is rejected with 401), discovers
 * the live OpenAI-style catalog, and constructs models through the shared
 * OpenAI-compatible Chat Completions factory. Credentials travel only in
 * request-scoped headers and never touch process state.
 */
import {
  AI_CATALOG_BOUNDS,
  AI_MODEL_PROTOCOLS,
...
export const metaAdapter: AiProviderAdapter = {
  kind: AI_PROVIDER_KINDS.META,
