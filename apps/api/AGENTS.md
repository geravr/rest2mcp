...
- AI provider credentials are persisted only through the identity-bound AES-256-GCM envelope in [lib/ai-crypto.ts](lib/ai-crypto.ts) (`AI_CREDENTIAL_SECRET`, distinct from `BETTER_AUTH_SECRET` and `MCP_CREDENTIAL_SECRET`). Never store, return, log, or project plaintext or ciphertext outside the service call that verifies or constructs a provider client; `AI_CREDENTIAL_SECRET` must never share key material with the other secrets.
- Provider traffic goes only through [lib/ai/provider-http.ts](lib/ai/provider-http.ts) and the adapters registered in [lib/ai/provider-registry.ts](lib/ai/provider-registry.ts): fixed HTTPS origins, bounded deadlines and response sizes, same-origin redirects only, and normalized `AiProviderRequestError` failures. Never accept provider base URLs, endpoints, or protocol overrides from client input.
- Model discovery is live and allowlist-free: adapters fetch the authenticated catalog, [lib/ai/catalog-normalize.ts](lib/ai/catalog-normalize.ts) enforces bounded entries, and models.dev enrichment can only annotate existing entries — never add models or resolve gateway routes by guessing. A gateway model without a resolvable route stays unsupported.
- Model selection persists only after the bounded structured-output smoke test and only when pre-call connection revisions are still current (verify in [services/ai-catalog-service.ts](services/ai-catalog-service.ts)). Readiness is bound to the verification fingerprint in [lib/ai/verification.ts](lib/ai/verification.ts); transient provider failures never erase a verified connection or selection, and every AI feature endpoint re-enforces readiness via `assertAiFeatureReady` instead of trusting the UI.
- The Mastra runtime boundary in [lib/ai/ai-runtime.ts](lib/ai/ai-runtime.ts) constructs request-scoped AI SDK model instances from decrypted database credentials. Never write credentials to `process.env`, provider singletons, Mastra storage, or browser state, and never let Mastra types leak into tRPC contracts or shared core types.
...
- Schema fields: `ENVIRONMENT`, `APP_NAME`, `APP_ORIGIN`, `DATABASE_URL`, `BETTER_AUTH_SECRET`, `RESEND_API_KEY`, `RESEND_EMAIL_FROM`. Secrets with dedicated key material: `MCP_CREDENTIAL_SECRET` (upstream MCP credentials), `AI_CREDENTIAL_SECRET` (account AI provider credentials). Optional: PostHog (`POSTHOG_*`), S3 (`STORAGE_S3_*`), `SUPER_ADMIN_EMAIL`, `MCP_MAX_TOOLS_PER_SERVER` (per-server tool cap; default `50`, accepted `1-500`). `API_ORIGIN` is consumed by the SPA proxy / marketing templates, not by [lib/env.ts](lib/env.ts).
